...
public class Subscriber<T> : IDisposable
{
    ...
    public bool NotifyAlways { get; }

    public Subscriber(Action<T> action, Action<Subscriber<T>> onDispose, bool notifyAlways)
    { _action = action; _onDispose = onDispose; NotifyAlways = notifyAlways; }
...
public class Subscriber<T1, T2> : IDisposable
{
    ...
    public Subscriber(Action<T1, T2> action, Action<Subscriber<T1, T2>> onDispose, bool notifyAlways)
    { _action = action; _onDispose = onDispose; NotifyAlways = notifyAlways; }
